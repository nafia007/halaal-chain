...
export default function IssuePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.account || !form.amount) {
      setStatus({ type: 'error', message: 'Account and amount are required' })
      return
    }
    setLoading(true)
    setStatus(null)
    try {
      ...
      setStatus({ type: 'success', message: `Issued! Tx: ${data.transactionHash}` })
      // Clear form after success
      setForm({ account: '', amount: '', metadata: '' })
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-black pt-24 pb-20 px-6 md:px-12 lg:px-20">
      <div className="max-w-[820px] mx-auto">
        ...
        <p className="text-white/50 text-[15px] mb-10 max-w-[520px]">
          Issue new HalaalCertificationToken to an address. Requires admin privileges.
        </p>
        ...
        {!address ? (
          <Card className="glass-card border-white/[0.08]">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-white/50 text-[15px] text-center mb-8 max-w-[360px]">
                Connect MetaMask to issue certification on the Polygon PoS network.
              </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <Card className="glass-card border-white/[0.08]">
              <CardHeader>
                <CardTitle className="text-white font-medium">Issue Certification</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="metadata" className="text-white/60 font-mono text-[13px]">
                    Metadata (optional)
                  </Label>
                  <Textarea
                    id="metadata"
                    value={form.metadata}
                    onChange={handleChange}
                    placeholder="Any additional data (e.g. certificate ID)"
        ...
        <div className="mt-8 p-4 rounded-xl border border-white/[0.06] bg-white/[0.02]">
          ...
          <p className="text-white/40 text-xs">
            Issue endpoint: <code className="text-white/60">POST /api/issue</code> (localhost:3001).
          </p>
        </div>
      </div>
    </div>
